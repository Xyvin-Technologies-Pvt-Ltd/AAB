      ...
      <div className="px-4 py-6 sm:px-0">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">{analytics.package.clientName}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
          <div className="bg-white rounded-lg shadow p-3">
            ...
            <p className="text-base font-semibold">{formatCurrency(analytics.summary?.revenue || 0)}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-3">
            <p className="text-xs text-gray-500 mb-1">Total Cost</p>
            <p className="text-base font-semibold">{formatCurrency(analytics.summary?.cost || 0)}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-3">
            <p className="text-xs text-gray-500 mb-1">Total Profit</p>
            <p className={`text-base font-semibold ${(analytics.summary?.profit || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <div className="bg-white rounded-lg shadow p-3">
            ...
            <p className="text-base font-semibold">{analytics.summary?.taskCount || 0}</p>
          </div>
        </div>

        {/* Charts Side by Side */}
        {analytics.monthlyTrends && analytics.monthlyTrends.length > 0 && (
          <div className={`grid grid-cols-1 ${analytics.package.type === 'RECURRING' ? 'lg:grid-cols-2' : 'lg:grid-cols-1'} gap-6 mb-6`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-lg shadow p-5">
            <h2 className="text-lg font-semibold mb-3">Top 3 Employees by Tasks</h2>
            {analytics.topEmployeesByTasks && analytics.topEmployeesByTasks.length > 0 ? (
              <div className="space-y-2">
                {analytics.topEmployeesByTasks.map((emp, idx) => (
                  <div key={emp.employeeId} className="flex items-center justify-between p-2.5 bg-gray-50 rounded">
                    <div>
                      <p className="text-sm font-medium">{emp.name}</p>
                      <p className="text-xs text-gray-500">{emp.taskCount} tasks ({emp.percentage}%)</p>
                    </div>
          ...
          <div className="bg-white rounded-lg shadow p-5">
            <h2 className="text-lg font-semibold mb-3">Top 3 Employees by Time</h2>
            {analytics.topEmployeesByTime && analytics.topEmployeesByTime.length > 0 ? (
              <div className="space-y-2">
                  ...
                  <div key={emp.employeeId} className="flex items-center justify-between p-2.5 bg-gray-50 rounded">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {emp.hours} hrs ({emp.percentage}%) • {formatCurrency(emp.cost)}
                      </p>
                    </div>
                    <Badge variant="secondary" className="text-xs">{idx + 1}</Badge>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          {/* Employee Breakdown */}
          {analytics.employeeBreakdown && analytics.employeeBreakdown.length > 0 && (
            <div className="bg-white rounded-lg shadow p-5">
              <h2 className="text-lg font-semibold mb-3">Employee Breakdown</h2>
